<template>
    ...
    <table id="list" class="table table-striped table-bordered table-hover">
        <thead>
            <tr>
                <th>가맹점 코드</th><th>입력 일시</th><th>입력 사용자ID</th>
                <th>가맹점 이름</th><th>가맹점 이름APP</th>
                <th>가맹점 대표자명</th><th>가맹점 전화번호</th>
            </tr>
        </thead>
        <tbody id="mngrsts" >
            <tr v-for="mngrst in mngrstlist.mngrsts" :key="mngrst.stCode">
                <td>{{mngrst.stCode}}</td>
                <td>{{mngrst.putDate}}</td>
                <td>{{mngrst.putUserId}}</td>
                <td>{{mngrst.stName}}</td>
                <td>{{mngrst.stNameApp}}</td>
                <td>{{mngrst.stOwner}}</td>
                <td>{{mngrst.stTel}}</td>
            </tr>
        </tbody>
    </table>
...
export default {
    ...
    computed : _.extend(
        {
            totalpage : function() {
                var totalcount = this.mngrstlist.totalcount;
                var pagesize = this.mngrstlist.pagesize;
    ...
    mounted : function() {
        ...
        this.$store.dispatch(Constant.FETCH_MNGRSTS, { pageno:page });
        this.$refs.pagebuttons.selected = page-1;
    },
    watch : {
        '$route' : function(to, from) {
            if (to.query.page && to.query.page != this.mngrstlist.pageno) {
    ...
    methods : {
        pageChanged : function(page) {
            this.$router.push({ name: 'mngrsts', query: { page: page }})
        }
    }
}
</script>
