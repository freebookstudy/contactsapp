...
export default {
    ...
    [Constant.FETCH_MNGRSTS] : (store, payload) => {
        var pageno;
        if (typeof payload ==="undefined" || typeof payload.pageno ==="undefined")
            pageno = 1;
        else 
            pageno = payload.pageno;
        var pagesize = store.state.mngrstlist.pagesize;
        store.dispatch(Constant.CHANGE_ISLOADING, { isloading:true });
        contactAPI.fetchMngrsts(pageno, pagesize)
        .then((response)=> {
            store.commit(Constant.FETCH_MNGRSTS, { mngrstlist: response.data });
            store.dispatch(Constant.CHANGE_ISLOADING, { isloading:false });
        })
    },
}